...
$query = "SELECT filename, COUNT(issue) as countIssue FROM $error_results_table_name GROUP BY filename";
$result = mysqli_query($GLOBALS['con'], $query);
if(!$result){
...
$query = "SELECT start_time, end_time, timediff(end_time, start_time) as difftime, FORMAT(rows_count,2) as rowscount, FORMAT(defects_found,2) as defectsfound FROM audit WHERE files like '%".$_GET['ticket']."%'";
$result = mysqli_query($GLOBALS['con'], $query);
if(!$result){
	die('Updating ID - fatal error'.mysqli_error($GLOBALS['con']));
}
